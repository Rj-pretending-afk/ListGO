...
- [x] 邀请码被使用后显示使用者 `@username`（可点击打开 profile）
- [x] 无可用码时显示"申请新邀请码"按钮（条件：所有码已用 且 无待审请求）
- [x] 已有待审请求时按钮变为"等待管理员审核"禁用状态

**邀请码申请系统（行政流程，独立）**
- [x] DB：`invite_requests` 表（id, requester_id, status, created_at, resolved_at, resolved_by）
- [x] 后端：`POST /invite-request` — 用户申请（校验无可用码 + 无重复申请）
- [x] 后端：`GET /admin/invite-requests` — 管理员查看待审请求（带申请者信息）
- [x] 后端：`PUT /admin/invite-requests/:id/accept` — 通过（自动生成新邀请码给申请者）
- [x] 后端：`PUT /admin/invite-requests/:id/reject` — 拒绝
- [x] 后端：`POST /admin/users/:userId/invite-code` — 管理员直接给某用户生成新码
- [x] 前端：管理员面板新增"邀请申请"标签页（含待审数量角标）
- [x] 前端：通知铃铛：有待审申请时在通知面板显示管理员条目（角标计入总数）

**戳系统（社交通知，独立）**
- [x] DB：`pokes` 表（id, sender_id, recipient_id, status, created_at）；`users` 表 ALTER 加 `poke_message TEXT`
- [x] 后端：`POST /pokes` — 发送戳（recipientId 必填）
- [x] 后端：`GET /pokes/inbox` — 获取我收到的戳（未读）
- [x] 后端：`PUT /pokes/:id/read` — 标记已读
- [x] 前端：用户可在个人设置设置"被戳时的提示"（文字 + emoji，≤50 字）
- [x] 前端：Header 通知铃铛角标（有未读戳/邀请时显示）
- [x] 前端：戳收件箱面板（点击铃铛打开，合并戳 + 清单邀请 + 管理员通知）
- [x] 前端：在 `/u/:username` 页添加"戳一下"按钮（显示对方的 poke_message）

#### 5.2 用户主页 `/u/:username`

- [x] 公开用户主页页面（路由 `/u/:username`）
- [x] 显示：头像 + 昵称 + 用户名 + bio
- [x] 自定义 bio（富文本一行，个人设置里编辑，存入 `users.bio` 字段）
- [x] 后端：`GET /users/:username/profile` 返回公开信息（含 poke_message）
- [x] 点击任意用户头像跳转该页（AvatarStack 里的头像、列表创建者 badge 等）
- [x] `users` 表 ALTER 加 `bio TEXT` 字段
- [x] 在此页显示"戳一下"按钮，完成戳系统的前端闭环

#### 5.3 基础完善

- [ ] 错误页 / 空状态 / 加载骨架
- [x] 网络断开提示（底部固定条，useNetworkStatus hook）
