...
export function FontSettingsPicker({ fontSettings, onFontChange }: FontSettingsPickerProps) {
  ...
  const panel = (
    <>
      ...
      <div className="fixed z-[201] rounded-xl shadow-2xl"
        style={{ top: panelPos.top, left: panelPos.left, width: '14rem', backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}
        onClick={e => e.stopPropagation()}>
        <div className="p-3 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold" style={{ color: 'var(--color-text)' }}>{t('titleFont')}</span>
            {hasFont && (
              <button className="text-xs hover:opacity-70" style={{ color: '#ef4444' }} onClick={() => onFontChange(undefined)}>{t('resetFont')}</button>
            )}
          </div>

          <div className="flex gap-1">
            {FONT_SIZES.map(({ key, value }) => (
              <button key={key} onClick={() => updateFont({ size: font.size === value ? undefined : value })}
                className="flex-1 py-1 rounded text-xs transition-colors"
                style={{ backgroundColor: font.size === value ? 'var(--color-primary)' : 'var(--color-border)', color: font.size === value ? 'white' : 'var(--color-text)' }}>
                {t(key)}
              </button>
            ))}
          </div>

          <select value={font.family || ''} onChange={e => updateFont({ family: e.target.value })}
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs" style={{ color: 'var(--color-text)', opacity: 0.5 }}>{t('fontColor')}</span>
            <label className="w-6 h-6 rounded overflow-hidden cursor-pointer border-2 relative flex-shrink-0"
              ...
              <input type="color" className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"
                value={font.color || '#ffffff'} onChange={e => updateFont({ color: e.target.value })} />
            </label>
            {font.color && (
              <button className="text-xs hover:opacity-70" style={{ color: '#ef4444' }} onClick={() => updateFont({ color: undefined })}>{t('clearColor')}</button>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="relative flex-shrink-0">
      <button ref={btnRef} onClick={handleOpen}
        ...
        style={{ color: hasFont ? (fontSettings?.color || 'var(--color-primary)') : 'var(--color-text)', opacity: hasFont ? 1 : 0.3, fontFamily: fontSettings?.family || undefined }}
        title={t('titleFont')}>
        A
      </button>
